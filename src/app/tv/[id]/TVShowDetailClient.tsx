...
export default function TVShowDetailClient({
  ...
  useEffect(() => {
    fetch(`/api/activities/tv/${tmdbId}`)
      ...
      .catch(() => {});
  }, [tmdbId, initialSeason]);

  // Fetch episodes & streams when selected season changes (if not the initial server-loaded season)
  useEffect(() => {
    if (selectedSeason === initialSeason && initialEpisodes.length > 0) {
      setEpisodes(initialEpisodes);
      return;
    }

    const cacheKey = `tv:${tmdbId}:s${selectedSeason}`;
    const cached = getClientCache<TVEpisode[]>(cacheKey);
    ...
    fetch(`/api/content/tv/${tmdbId}/season/${selectedSeason}`)
      ...
      .then((data) => {
        if (data?.ok && data.data?.episodes) {
          ...
          setClientCache(cacheKey, data.data.episodes);
        }
      })
      .catch((err) => console.error(err))
      .finally(() => setLoadingSeason(false));
  }, [tmdbId, selectedSeason, initialSeason, initialEpisodes]);

  // Fetch stream URLs when episode or season changes during playback
  useEffect(() => {
    if (selectedSeason === initialSeason && selectedEpisode === 1 && initialStreams.length > 0) {
    ...
    fetch(`/api/streams/tv/${tmdbId}/${selectedSeason}/${selectedEpisode}`)
      ...
      .catch((err) => console.error(err));
  }, [tmdbId, selectedSeason, selectedEpisode, initialSeason, initialStreams]);
  ...
  return (
    <div style={{ maxWidth: 1360, margin: "0 auto", padding: "1rem 1rem 5rem 1rem" }}>
      ...
      <div id="episodes-selector" className="panel" style={{ padding: "1.25rem", marginBottom: "2rem", background: "var(--bg-surface)" }}>
        ...
        >
          {loadingSeason ? (
            <div style={{ padding: "2rem", textAlign: "center", gridColumn: "1 / -1", color: "var(--text-muted)", fontFamily: "var(--font-mono)", fontSize: "0.8rem" }}>
              LOADING_SEASON_EPISODES...
            </div>
          ) : (
            episodes.map((ep) => {
